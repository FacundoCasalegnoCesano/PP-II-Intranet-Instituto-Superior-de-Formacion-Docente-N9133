...
vi.mock('../api/academicRecordApi', () => ({
  ...
  fetchAcademicRecordTrajectory: vi.fn().mockResolvedValue({
    ...
    materias: [{ materia: { id: 5, nombre: 'Pedagogía' }, estado: 'REGULAR', plan: { anio: 1 }, asistencia: { porcentaje: 80 }, regularidad: { hasta: '2027-12-31T00:00:00.000Z', vencida: false }, definitiva: null }],
  }),
}))
...
describe('AcademicRecordView', () => {
  it('renders the integral backend trajectory for the selected career', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Mi trayectoria académica' })).toBeVisible()
    await screen.findByText('Pedagogía')
    expect(screen.getByLabelText('Carrera')).toHaveValue('3')
    expect(screen.getByText('Pedagogía')).toBeVisible()
