...
describe('SubjectEnrollmentsView', () => {
  it('verifies a subject before showing enrollment confirmation and links to official schedules', async () => {
    api.available.mockResolvedValue([{ id: 5, nombre: 'PedagogÃ­a', curso: { anio: 1 }, carreras: [{ id: 3, nombre: 'Profesorado' }], modalidad: 'SEMIPRESENCIAL', yaInscripto: false, yaAprobada: false, cumpleCorrelativas: true, correlativasPendientes: [], habilitada: true }])
    api.verify.mockResolvedValue({ puedeInscribirse: true, materia: { id: 5, nombre: 'PedagogÃ­a', modalidad: 'SEMIPRESENCIAL', yaInscripto: false, yaAprobada: false, cumpleCorrelativas: true, correlativasPendientes: [], habilitada: true } })
    ...
    expect(await screen.findByRole('alertdialog', { name: 'Confirmar inscripciÃ³n' })).toBeVisible()
